    ...
    public partial class Form1 : Form
    {
        ...
        private async Task CheckForUpdatesAsync()
        {
            try
            {
                ...
                if (updateInfo == null)
                {
                    // Обновлений нет - показываем ненавязчивое уведомление
                    this.Invoke(new Action(() => ShowUpdateNotification("У вас самая новая версия", false)));
                }
                else
                {
                    // Есть обновление - показываем диалог
                    this.Invoke(new Action(() => ShowUpdateDialog(updateInfo, updateService)));
                }
