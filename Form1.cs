...
namespace ZontSpecExtractor
{
    ...
    public record PredefinedMasterConfig
    {
        ...
        // === ДОБАВЛЕННЫЕ ПОЛЯ ===
        public string FieldName { get; set; } = "";  // Имя свойства (Shape Data)
        public string FieldValue { get; set; } = ""; // Значение для записи
                                                     // ========================
    ...
    /// <summary>
    /// Класс для хранения настроек Telegram получателя
    /// </summary>
    ...
    public static class AppSettings
    {
        ...
        // === НОВОЕ: Список получателей Telegram ===
        public static List<TelegramRecipient> TelegramRecipients { get; set; } = new List<TelegramRecipient>();
            ...
            try
            {
                if (File.Exists(SettingsFilePath))
                {
                    ...
                    if (settings != null)
                    {
                        ...
                        // === НОВОЕ: Загрузка Telegram получателей ===
                        if (settings.TryGetValue("TelegramRecipients", out var tr))
                        {
                            TelegramRecipients = JsonSerializer.Deserialize<List<TelegramRecipient>>(tr.ToString()) ?? new List<TelegramRecipient>();
                        }
                        else
                        {
                            // Миграция: если есть старые настройки, конвертируем их
                            if (settings.TryGetValue("TelegramToken", out var token) || settings.TryGetValue("TelegramChatId", out var chatId))
                            {
                                var oldToken = settings.TryGetValue("TelegramToken", out var t) ? t.ToString() : "";
                                var oldChatId = settings.TryGetValue("TelegramChatId", out var chatIdVar) ? chatIdVar.ToString() : "";
                                
                                if (!string.IsNullOrEmpty(oldToken) && !string.IsNullOrEmpty(oldChatId))
                                {
                                    TelegramRecipients = new List<TelegramRecipient>
                                    {
                                        new TelegramRecipient { BotToken = oldToken, ChatId = oldChatId }
                                    };
                                }
                            }
                        }
        ...
        public static void Save()
        {
            try
            {
                var settings = new Dictionary<string, object>
                {
                    ...
                    { "Search", SearchConfig },
                    { "TelegramRecipients", TelegramRecipients } // === НОВОЕ: Сохранение Telegram получателей ===
                };
    ...
    public class GeneralSettingsForm : Form
    {
        ...
        // === НОВОЕ: DataGrid для Telegram получателей ===
        private DataGridView _dgvTelegramRecipients;
        ...
        private void SetupUI()
        {
            ...
            // 4. НОВАЯ ВКЛАДКА: Настройки Telegram
            var telegramPage = new TabPage("4. Настройки Telegram") { Padding = new Padding(10) };
            telegramPage.Controls.Add(CreateTelegramConfigPanel());
            tabControl.TabPages.Add(telegramPage);

            // 5. НОВАЯ ВКЛАДКА: Обновления
            var updatePage = new TabPage("5. Обновления") { Padding = new Padding(10) };
            updatePage.Controls.Add(CreateUpdateConfigPanel());
            tabControl.TabPages.Add(updatePage);
            ...
            btnSave.Click += (s, e) =>
            {
                ...
                // === НОВОЕ: Сохраняем Telegram получателей ===
                if (_dgvTelegramRecipients.DataSource is System.ComponentModel.BindingList<TelegramRecipient> telegramList)
                {
                    // Фильтруем пустые записи (где нет Token или ChatID)
                    AppSettings.TelegramRecipients = telegramList
                        .Where(r => !string.IsNullOrWhiteSpace(r.BotToken) && !string.IsNullOrWhiteSpace(r.ChatId))
                        .ToList();
                }
        ...
        private TabPage CreateSingleSheetSettingsPage(string title, VisioConfiguration cfg)
        {
            ...
            dgvFixed.Columns.Add(new DataGridViewTextBoxColumn
            {
                HeaderText = "Поле (Shape Data)",
                DataPropertyName = "FieldName", // Должно совпадать со свойством в PredefinedViewModel
                Width = 100
            });
            ...
            {
                HeaderText = "Значение",
                DataPropertyName = "FieldValue", // Должно совпадать со свойством в PredefinedViewModel
                Width = 100
            });
        ...
        public class PredefinedViewModel
        {
            ...
            public PredefinedMasterConfig ToConfig()
            {
                // Собираем координаты
                string coords = $"{X},{Y}";
                // Если X или Y пустые, можно добавить проверку, но обычно там "0" по дефолту
            ...
            public PredefinedViewModel(PredefinedMasterConfig cfg)
            {
                ...
                // === ВОССТАНАВЛИВАЕМ НОВЫЕ ПОЛЯ ИЗ КОНФИГА ===
        ...
        /// <summary>
        /// Создает панель для настройки Telegram получателей
        /// </summary>
        private Panel CreateTelegramConfigPanel()
        {
            var panel = new Panel { Dock = DockStyle.Fill };
            var layout = new TableLayoutPanel
            {
                ...
                RowCount = 3,
                RowStyles = {
                    new RowStyle(SizeType.Absolute, 30), // Заголовок
                    new RowStyle(SizeType.Percent, 100), // DataGrid
                    new RowStyle(SizeType.Absolute, 40)  // Кнопки управления
                }
            };
            ...
            { 
                Text = "Получатели Telegram (Bot Token и Chat ID):", 
                Dock = DockStyle.Bottom, 
                Font = new Font("Segoe UI", 9, FontStyle.Bold) 
            }, 0, 0);

            // DataGrid для получателей
            _dgvTelegramRecipients = new DataGridView
            {
                Dock = DockStyle.Fill,
                AutoGenerateColumns = false,
                BackgroundColor = Color.White,
            ...
            // Колонка Bot Token
            _dgvTelegramRecipients.Columns.Add(new DataGridViewTextBoxColumn
            {
                HeaderText = "Bot Token",
                DataPropertyName = "BotToken",
                AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill,
                MinimumWidth = 200
            });
            ...
            // Инициализируем список получателей (если пусто, добавляем один пустой элемент)
            var recipientsList = AppSettings.TelegramRecipients ?? new List<TelegramRecipient>();
            if (recipientsList.Count == 0)
            {
                recipientsList.Add(new TelegramRecipient());
            }

            _dgvTelegramRecipients.DataSource = new System.ComponentModel.BindingList<TelegramRecipient>(recipientsList);
            layout.Controls.Add(_dgvTelegramRecipients, 0, 1);
            ...
            btnAdd.Click += (s, e) =>
            {
                if (_dgvTelegramRecipients.DataSource is System.ComponentModel.BindingList<TelegramRecipient> list)
                {
                    list.Add(new TelegramRecipient());
                }
            };
            ...
            btnRemove.Click += (s, e) =>
            {
                if (_dgvTelegramRecipients.SelectedRows.Count > 0)
                {
                    foreach (DataGridViewRow row in _dgvTelegramRecipients.SelectedRows)
                    {
                        if (!row.IsNewRow)
                        {
                            _dgvTelegramRecipients.Rows.Remove(row);
                        }
                    }
                }
            };
        ...
        /// <summary>
        /// Создает панель для настройки обновлений
        /// </summary>
        private Panel CreateUpdateConfigPanel()
        {
            var panel = new Panel { Dock = DockStyle.Fill };
            var layout = new TableLayoutPanel
            {
                ...
                RowStyles = {
                    ...
                    new RowStyle(SizeType.Absolute, 100), // Информация о версии
                    new RowStyle(SizeType.Absolute, 50), // Кнопка
                    new RowStyle(SizeType.Percent, 100)  // Пустое пространство
                }
            };
            ...
            // Информация о версии
            var versionPanel = new Panel { Dock = DockStyle.Fill };
            ...
            btnCheckUpdates.Click += async (s, e) =>
            {
                try
                {
                    ...
                    var updateService = new GitHubUpdateService("AcId4848", "ZontSpecExtractor");
                    ...
                    {
                        MessageBox.Show(
                            $"У вас установлена самая новая версия ({currentVersion}).",
                    ...
                    {
                        ShowUpdateDialogInSettings(updateInfo);
                    }
                }
                catch (Exception ex)
                {
                    LoggingSystem.LogException("GeneralSettingsForm", "CheckUpdates_Click", ex);
                    MessageBox.Show(
            ...
            layout.Controls.Add(btnCheckUpdates, 0, 2);
        ...
        /// <summary>
        /// Показывает диалог обновления из настроек
        /// </summary>
        private void ShowUpdateDialogInSettings(UpdateInfo updateInfo)
        {
            try
            {
                var currentVersion = GitHubUpdateService.GetCurrentVersion();
                var message = $"Доступна новая версия {updateInfo.Version} (текущая: {currentVersion}).\n\n" +
                             $"Хотите обновиться?\n\n" +
                             (!string.IsNullOrEmpty(updateInfo.ReleaseNotes) 
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        ...
                        try
                        {
                            updateService = new GitHubUpdateService("AcId4848", "ZontSpecExtractor");
                            var settingsPath = Path.Combine(
                                Path.GetDirectoryName(Application.ExecutablePath) ?? "",
                                "app_settings.json"
                            );

                            var success = await updateService.DownloadAndInstallUpdateAsync(updateInfo, settingsPath);
                                    ...
                                    this.BeginInvoke(new Action(() =>
                                    {
                                        if (!this.IsDisposed)
                                        {
                                            MessageBox.Show(
                                                ...
                                                "Приложение закроется через несколько секунд.",
                                                "Обновление",
                                                MessageBoxButtons.OK,
                                                MessageBoxIcon.Information
                                            );
                            ...
                            else
                            {
                                if (this.IsHandleCreated && !this.IsDisposed)
                                {
                                    this.BeginInvoke(new Action(() =>
                                    {
                                        ...
                                        {
                                            MessageBox.Show(
                                                "Не удалось установить обновление. Попробуйте позже или скачайте вручную.",
                                                "Ошибка",
                                                MessageBoxButtons.OK,
                        ...
                        {
                            LoggingSystem.LogException("GeneralSettingsForm", "ShowUpdateDialogInSettings", ex);
                            if (this.IsHandleCreated && !this.IsDisposed)
                            {
                                this.BeginInvoke(new Action(() =>
                                {
    ...
    public partial class Form1 : Form
    {
        ...
        // Debug панель
        private DebugPanelForm? _debugPanel = null;
        private Thread? _debugPanelThread = null;
        private System.Windows.Forms.Timer? _hangDetectionTimer = null;
        private DateTime _lastUIResponse = DateTime.Now;
        private bool _hangDetectionEnabled = true;
        ...
        {
            // Останавливаем таймер обнаружения зависаний
            _hangDetectionTimer?.Stop();
            _hangDetectionTimer?.Dispose();
            _hangDetectionTimer = null;
            
            // Закрываем Debug панель, если открыта
            if (_debugPanel != null && !_debugPanel.IsDisposed)
            {
                _debugPanel.Invoke(new Action(() => _debugPanel.Close()));
            }
        }
        ...
        private void InitializeLoggingSystem()
        {
            try
            {
                ...
                LoggingSystem.Initialize(LogLevel.DEBUG);

                // Настраиваем Telegram (множественные получатели)
                if (AppSettings.TelegramRecipients != null && AppSettings.TelegramRecipients.Count > 0)
                {
                    var multiTelegramLogger = new MultiTelegramLogger(AppSettings.TelegramRecipients, LogLevel.WARNING);
                    LoggingSystem.AddHandler(multiTelegramLogger);
                    LoggingSystem.Log(LogLevel.INFO, "Form1", "InitializeLoggingSystem", 
                        $"Telegram logger configured for {AppSettings.TelegramRecipients.Count} recipient(s)");
                }
                else
                {
                    LoggingSystem.Log(LogLevel.INFO, "Form1", "InitializeLoggingSystem", 
                ...
                // Debug панель НЕ запускается автоматически - только по кнопке или при зависании
                // Инициализируем таймер для обнаружения зависаний
                InitializeHangDetection();

                LoggingSystem.Log(LogLevel.INFO, "Form1", "InitializeLoggingSystem", "Logging system initialized successfully");
            }
            catch (Exception ex)
            {
                // Критично: ошибка инициализации логирования не должна ронять приложение
                System.Diagnostics.Debug.WriteLine($"Logging initialization error: {ex.Message}");
            }
        }

        private void InitializeHangDetection()
                {
                    try
                    {
                _lastUIResponse = DateTime.Now;
                _hangDetectionTimer = new System.Windows.Forms.Timer();
                _hangDetectionTimer.Interval = 2000; // Проверяем каждые 2 секунды
                _hangDetectionTimer.Tick += HangDetectionTimer_Tick;
                _hangDetectionTimer.Start();

                // Отслеживаем активность UI
                this.MouseMove += (s, e) => _lastUIResponse = DateTime.Now;
                this.KeyDown += (s, e) => _lastUIResponse = DateTime.Now;
                    }
                    catch (Exception ex)
                    {
                System.Diagnostics.Debug.WriteLine($"Hang detection initialization error: {ex.Message}");
            }
        }

        private void HangDetectionTimer_Tick(object? sender, EventArgs e)
        {
            if (!_hangDetectionEnabled) return;

            try
            {
                // Не проверяем зависания, если открыто модальное окно (ShowDialog блокирует UI поток)
                if (this.Modal)
                {
                    return;
                }

                // Простая проверка без блокировки UI потока
                // Просто проверяем, когда был последний ответ от UI
                var timeSinceLastResponse = (DateTime.Now - _lastUIResponse).TotalSeconds;
                
                if (timeSinceLastResponse > 60) // Если UI не отвечает более 60 секунд (1 минута) - учитываем долгое открытие настроек
                {
                    // Автоматически открываем Debug панель
                    if (_debugPanel == null || _debugPanel.IsDisposed)
                    {
                        LoggingSystem.Log(LogLevel.WARNING, "Form1", "HangDetectionTimer_Tick", 
                            $"Обнаружено зависание UI (не отвечает {timeSinceLastResponse:F1} сек). Автоматически открываю Debug панель.");
                        OpenDebugPanel();
                    }
                }
            }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"Hang detection error: {ex.Message}");
            }
        }
        ...
        private void OpenDebugPanel()
        {
            try
            {
                // Если панель уже открыта, просто показываем её
                if (_debugPanel != null && !_debugPanel.IsDisposed)
                {
                    _debugPanel.BringToFront();
                    _debugPanel.WindowState = FormWindowState.Normal;
                    _debugPanel.Show();
                    return;
                }

                // Создаем новую панель в отдельном потоке
                _debugPanelThread = new Thread(() =>
                {
                    try
                    {
                        _debugPanel = new DebugPanelForm();
                        Application.Run(_debugPanel);
                    }
                    catch (Exception ex)
                    {
                        LoggingSystem.LogException("Form1", "OpenDebugPanel", ex);
                        System.Diagnostics.Debug.WriteLine($"Debug panel error: {ex.Message}");
                    }
                    finally
                    {
                        _debugPanel = null;
                        _debugPanelThread = null;
                    }
                })
                {
                    IsBackground = false,
                    Name = "DebugPanelThread"
                };
                _debugPanelThread.SetApartmentState(ApartmentState.STA);
                _debugPanelThread.Start();

                LoggingSystem.Log(LogLevel.INFO, "Form1", "OpenDebugPanel", "Debug панель открыта");
            }
            catch (Exception ex)
            {
                LoggingSystem.LogException("Form1", "OpenDebugPanel", ex);
                MessageBox.Show($"Не удалось открыть Debug панель: {ex.Message}", "Ошибка", 
        ...
        {
            // Временно отключаем hang detection при открытии модального окна
            var wasEnabled = _hangDetectionEnabled;
            _hangDetectionEnabled = false;
            ...
            {
                // Восстанавливаем hang detection и обновляем время последнего ответа
                _hangDetectionEnabled = wasEnabled;
                _lastUIResponse = DateTime.Now;
            }
        }
        ...
        private async Task CheckForUpdatesAsync()
        {
            try
            {
                // Используем репозиторий из задачи
                var updateService = new GitHubUpdateService("AcId4848", "ZontSpecExtractor");
                var updateInfo = await updateService.CheckForUpdatesAsync();
                updateService.Dispose();
                ...
                if (updateInfo == null)
                {
                    ...
                    this.Invoke(new Action(() => ShowUpdateNotification("Не удалось проверить обновления", true)));
                }
                else if (!updateInfo.HasUpdate)
                {
                    // У пользователя актуальная версия
                    this.Invoke(new Action(() => ShowUpdateNotification("У вас актуальная версия", false)));
                }
                else
                {
                    // Есть обновление - показываем диалог
                    this.Invoke(new Action(() => ShowUpdateDialog(updateInfo, null)));
                }
            }
            catch (Exception ex)
            {
                LoggingSystem.Log(LogLevel.WARNING, "Form1", "CheckForUpdatesAsync", 
                    $"Failed to check updates: {ex.Message}");
            }
        }

        /// <summary>
        /// Показывает ненавязчивое уведомление в нижней части окна (мигает и исчезает)
        /// </summary>
        private void ShowUpdateNotification(string message, bool isError)
        {
            try
            {
            ...
            {
                LoggingSystem.Log(LogLevel.WARNING, "Form1", "ShowUpdateNotification", 
        ...
        /// </summary>
        private void ShowUpdateDialog(UpdateInfo updateInfo, GitHubUpdateService? updateService)
        {
            try
            {
                var currentVersion = GitHubUpdateService.GetCurrentVersion();
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        GitHubUpdateService? downloadService = null;
                        try
                        {
                            // Создаем новый сервис для загрузки
                            downloadService = new GitHubUpdateService("AcId4848", "ZontSpecExtractor");
                            ...
                            var success = await downloadService.DownloadAndInstallUpdateAsync(updateInfo, settingsPath);
                            ...
                            else
                            {
                                this.Invoke(new Action(() =>
                                {
                                    UpdateStatus("Ошибка при установке обновления.");
                                    MessageBox.Show(
                                        "Не удалось установить обновление. Попробуйте позже или скачайте вручную.",
                                        "Ошибка",
                                        MessageBoxButtons.OK,
                        ...
                        {
                            LoggingSystem.LogException("Form1", "ShowUpdateDialog", ex);
                            if (this.IsHandleCreated && !this.IsDisposed)
                            {
                                this.Invoke(new Action(() =>
                                {
        ...
        private void SetupUI()
        {
            ...
            // Используем TableLayoutPanel для правильного расположения названия файла и кнопок
            var headerPanel = new TableLayoutPanel
            {
                ...
                ColumnStyles =
                {
                    new ColumnStyle(SizeType.Percent, 100F), // Название файла занимает оставшееся пространство
                    new ColumnStyle(SizeType.AutoSize)       // Кнопки занимают только необходимое место
                }
            };
            ...
            lblFileInfo = new Label
            {
                ...
                TextAlign = ContentAlignment.MiddleLeft,
                Dock = DockStyle.Fill,
                AutoEllipsis = true, // Обрезает текст с многоточием, если не помещается
                Margin = new Padding(0, 0, 10, 0) // Отступ справа от кнопок
            };
            ...
            var btnExportExcel = CreateStyledButton("Экспорт в Excel", System.Drawing.Color.FromArgb(52, 152, 219), System.Drawing.Color.White);
            btnExportExcel.Click += BtnExportExcel_Click;
            btnExportExcel.Image = originalExcelImage.GetThumbnailImage(ICON_SIZE, ICON_SIZE, null, IntPtr.Zero);
            btnExportExcel.ImageAlign = ContentAlignment.MiddleLeft;
            btnExportExcel.TextAlign = ContentAlignment.MiddleCenter;
            btnExportExcel.Height = 35;
            ...
            // Кнопка для запуска Debug панели
            var btnDebug = CreateStyledButton("🔴 Отладка", System.Drawing.Color.FromArgb(220, 53, 69), System.Drawing.Color.White);
            btnDebug.Click += BtnDebug_Click;

            buttonFlowPanel.Controls.Add(btnLoad);
            buttonFlowPanel.Controls.Add(btnExportExcel);
            ...
            buttonFlowPanel.Controls.Add(btnDebug);

            // Добавляем элементы в TableLayoutPanel: сначала название файла, потом кнопки
            headerPanel.Controls.Add(lblFileInfo, 0, 0);
            headerPanel.Controls.Add(buttonFlowPanel, 1, 0);
            ...
            try
            {
                using (var openFileDialog = new OpenFileDialog())
                {
                    ...
                    if (openFileDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        LoggingSystem.Log(LogLevel.INFO, "Form1", "BtnLoadFile_Click", 
                            $"File selected: {openFileDialog.FileName}",
                            null,
                            new Dictionary<string, object> { { "FilePath", openFileDialog.FileName } });
                        ...
                        {
                            LoggingSystem.Log(LogLevel.WARNING, "Form1", "BtnLoadFile_Click", 
                        ...
                        LoggingSystem.Log(LogLevel.INFO, "Form1", "BtnLoadFile_Click", 
                            $"Found {allSheetNames.Count} sheets: {string.Join(", ", allSheetNames)}");
                        ...
                        using (var sheetForm = new SheetSelectionForm(allSheetNames, AppSettings.SearchConfig.TargetSheetNames))
                        {
                            ...
                            if (sheetForm.ShowDialog() == DialogResult.OK)
                            {
                                ...
                                LoggingSystem.Log(LogLevel.INFO, "Form1", "BtnLoadFile_Click", 
                                    $"Sheets selected: {string.Join(", ", sheetForm.SelectedSheets)}");
                            ...
                            {
                                LoggingSystem.Log(LogLevel.INFO, "Form1", "BtnLoadFile_Click", "File loading cancelled by user");
                    ...
                    {
                        LoggingSystem.Log(LogLevel.INFO, "Form1", "BtnLoadFile_Click", "File dialog cancelled");
                    }
                }
            }
            catch (Exception ex)
            {
                LoggingSystem.LogException("Form1", "BtnLoadFile_Click", ex);
                throw;
            }
            finally
            {
        ...
        private void CreateTableClick(object? sender, EventArgs e)
        {
            try
            {
                LoggingSystem.Log(LogLevel.INFO, "Form1", "CreateTableClick", "=== НАЧАЛО СОЗДАНИЯ ТАБЛИЦЫ ===");

                if (!_rawHits.Any())
                {
                    LoggingSystem.Log(LogLevel.WARNING, "Form1", "CreateTableClick", "Попытка создать таблицу без данных");
                ...
                if (!rowsData.Any())
                {
                    LoggingSystem.Log(LogLevel.WARNING, "Form1", "CreateTableClick", "Не найдено данных для формирования таблицы");
                ...
                LoggingSystem.Log(LogLevel.INFO, "Form1", "CreateTableClick", $"Таблица создана. Строк: {rowsData.Count}");
                tableForm.ShowDialog();
            }
            catch (Exception ex)
            {
                LoggingSystem.LogException("Form1", "CreateTableClick", ex);
                MessageBox.Show($"Ошибка создания таблицы: {ex.Message}", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        /// <summary>
        /// Обработчик кнопки экспорта главной таблицы в Excel
        /// </summary>
        private void BtnExportExcel_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Нет данных для экспорта. Сначала загрузите и обработайте файл.", 
        ...
        /// <summary>
        /// Универсальный метод экспорта DataGridView в Excel
        /// </summary>
        private void ExportDataGridViewToExcel(DataGridView dgv)
        {
            ...
            {
                string outputPath;
                string defaultFileName = "Таблица.xlsx";
                
                // Определяем предложенное имя файла
                if (!string.IsNullOrEmpty(AppSettings.LastLoadedFilePath))
                {
                    ...
                    defaultFileName = $"{sourceFileNameWithoutExt} [таблица].xlsx";
                }
                ...
                using (var saveDialog = new SaveFileDialog())
                {
                    ...
                    saveDialog.Title = "Сохранить таблицу в Excel";
                ...
                {
                    var worksheet = package.Workbook.Worksheets.Add("Таблица");
                    ...
                    UpdateStatus($"Таблица экспортирована: {Path.GetFileName(outputPath)}");
        ...
        /// <summary>
        /// Экспортирует таблицу в Excel файл (для диалога создания таблицы)
        /// </summary>
        private void ExportTableToExcel(DataGridView dgv, object rowsData)
        {
            try
            {
                ...
                // Формируем имя файла: исходное имя + [таблица].xlsx
                var sourceFileInfo = new FileInfo(AppSettings.LastLoadedFilePath);
                var sourceFileNameWithoutExt = Path.GetFileNameWithoutExtension(sourceFileInfo.Name);
                var outputDirectory = sourceFileInfo.DirectoryName ?? Environment.CurrentDirectory;
                var outputFileName = $"{sourceFileNameWithoutExt} [таблица].xlsx";
                ...
                {
                    outputFileName = $"{sourceFileNameWithoutExt} [таблица] ({counter}).xlsx";
                ...
                using (var package = new ExcelPackage())
                {
                    ...
                    UpdateStatus($"Таблица экспортирована: {outputFileName}");
                }
            }
            catch (Exception ex)
            {
        ...
        private async void LoadFiles(string[] filePaths)
        {
            ...
            try
            {
                ...
                if (filePaths.Length > 0)
                {
                    ...
                    // Обновляем заголовок формы с именем файла
                    this.Text = $"{fileName} - ZONT Spec Extractor";
                }
                else
                {
                ...
                try
                {
                    ...
                    // 2. Обновляем таблицу на главной форме (простой просмотр)
                    UpdateDataGridView();
                    ShowResultMessage(_rawHits.Count);

                    // 3. Создаем полную таблицу в основном окне (как в Form1 (13).cs)
                    CreateMainTableAfterLoad();
                ...
                catch (Exception ex)
                {
                    ...
                    // Сбрасываем заголовок при ошибке
                    this.Text = "ZONT Spec Extractor";
                    lblFileInfo.Text = "Файл не выбран";
                }
            }
        ...
        // =================================================================================
        // МЕТОД: Создание полной таблицы в основном окне после загрузки
        // =================================================================================
        private void CreateMainTableAfterLoad()
        {
            if (!_rawHits.Any()) return;
            ...
            if (mainDgv != null)
            {
                ...
                bool hasForFooter = finalQueue.Any(x => (x.Name ?? "").Contains("Управл.", StringComparison.OrdinalIgnoreCase));
        ...
        private void UpdateDataGridView()
        {
            ...
            foreach (var rowData in data)
            {
                ...
                // Маппинг данных из GroupRawHitsForVisio в формат для отображения
                row.Cells.Add(new DataGridViewTextBoxCell { Value = rowNum.ToString() });
                row.Cells.Add(new DataGridViewTextBoxCell { Value = rowData.GetValueOrDefault("Наименование", string.Empty) });
                row.Cells.Add(new DataGridViewTextBoxCell { Value = rowData.GetValueOrDefault("Количество", string.Empty) });
            ...
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    foreach (var path in allStencils)
                    {
                        ...
                        // Сначала проверяем, не открыт ли уже этот документ в текущем приложении
                        try
                        {
                            foreach (Visio.Document existingDoc in visioApp.Documents)
                            {
                                if (existingDoc.FullName.Equals(path, StringComparison.OrdinalIgnoreCase) || 
                                    existingDoc.Name.Equals(fileName, StringComparison.OrdinalIgnoreCase))
                                {
                                    LoggingSystem.Log(LogLevel.INFO, "Form1", "OpenVisioClick", $"Трафарет уже открыт: {fileName}");
                                    opened = true;
                                    break;
                                }
                            }
                        ...
                        // Пытаемся открыть трафарет
                        try
                        {
                            visioApp.Documents.OpenEx(path, (short)Visio.VisOpenSaveArgs.visOpenDocked);
                            LoggingSystem.Log(LogLevel.INFO, "Form1", "OpenVisioClick", $"Трафарет открыт: {fileName}");
                            opened = true;
                        }
                        catch (System.Runtime.InteropServices.COMException comEx)
                        {
                            // Если файл уже открыт в другом экземпляре, пробуем разные варианты
                            try
                            {
                                // Вариант 1: Только для чтения
                                visioApp.Documents.OpenEx(path, (short)(Visio.VisOpenSaveArgs.visOpenDocked | Visio.VisOpenSaveArgs.visOpenRO));
                                LoggingSystem.Log(LogLevel.WARNING, "Form1", "OpenVisioClick", $"Трафарет открыт только для чтения: {fileName}");
                                opened = true;
                            }
                            catch
                            {
                                try
                                {
                                    // Вариант 2: Без докирования
                                    visioApp.Documents.OpenEx(path, (short)Visio.VisOpenSaveArgs.visOpenHidden);
                                    LoggingSystem.Log(LogLevel.WARNING, "Form1", "OpenVisioClick", $"Трафарет открыт скрыто: {fileName}");
                                    opened = true;
                                }
                                catch (Exception ex2)
                                {
                                    LoggingSystem.Log(LogLevel.ERROR, "Form1", "OpenVisioClick", 
                                        $"Не удалось открыть трафарет {fileName}. Файл открыт в другом экземпляре Visio. Закройте файл в Visio и попробуйте снова. Ошибка: {comEx.Message}");
                                    // Продолжаем работу без этого трафарета
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            LoggingSystem.Log(LogLevel.ERROR, "Form1", "OpenVisioClick", $"Ошибка при открытии трафарета {fileName}: {ex.Message}");
                        }
                    }

                    // Проверяем, что трафареты действительно открыты
                    var openedStencils = new List<string>();
                    foreach (Visio.Document d in visioApp.Documents)
                    {
                        if (d.Type == Visio.VisDocumentTypes.visTypeStencil)
                        {
                            openedStencils.Add(d.Name);
                        }
                    }
                    LoggingSystem.Log(LogLevel.INFO, "Form1", "OpenVisioClick", $"Открыто трафаретов: {openedStencils.Count}");
        ...
        private void GeneratePageDirectly(Visio.Document doc, string pageName, List<RawExcelHit> hits, VisioConfiguration config)
        {
            ...
            try
            {
                ...
                LoggingSystem.Log(LogLevel.INFO, "Form1", "GeneratePageDirectly", $"Страница '{pageName}' создана/найдена. Обрабатывается {hits.Count} элементов.");
            }
            catch (Exception ex)
            {
                LoggingSystem.Log(LogLevel.ERROR, "Form1", "GeneratePageDirectly", $"Ошибка при создании страницы '{pageName}': {ex.Message}");
                return;
            }
                ...
                foreach (var fixedItem in config.PredefinedMasterConfigs)
                {
                    ...
                    for (int i = 0; i < fixedItem.Quantity; i++)
                    {
                        ...
                        if (shp != null)
                        {
                            ...
                            // ====================================================================
                            // 3. НОВОЕ: ЗАПОЛНЕНИЕ ПОЛЕЙ
                            // Если указано имя поля И значение, записываем их в данные фигуры
                            // ====================================================================
                            ...
                            LoggingSystem.Log(LogLevel.INFO, "Form1", "GeneratePageDirectly", $"Фигура '{fixedItem.MasterName}' создана на позиции ({x}, {y})");
                        }
                        else
                        {
                            LoggingSystem.Log(LogLevel.WARNING, "Form1", "GeneratePageDirectly", $"Не удалось создать фигуру '{fixedItem.MasterName}'");
                        }
                    }
                }
            }
            ...
            if (config.SearchRules != null && config.SearchRules.Any() && hits != null && hits.Any())
            {
                ...
                int blockLabelCounter = 3; // Клеммы (3, 4, 5...) - начинаем с 3
                int sensorCounter = 1;     // Датчики (1, 2, 3...)

                // Ключевые слова для разделения нумерации клемм
                string[] splitKeywords = new[] { "Перекл.", "Смеситель", "Управл.", "давления", "кран", "Шина"};
                ...
                foreach (var hit in hits)
                {
                    ...
                    if (matchedRule != null && !string.IsNullOrWhiteSpace(matchedRule.VisioMasterName))
                    {
                        ...
                        for (int i = 0; i < countToDraw; i++)
                        {
                            ...
                            if (shp != null)
                            {
                                LoggingSystem.Log(LogLevel.INFO, "Form1", "GeneratePageDirectly", $"Фигура '{matchedRule.VisioMasterName}' создана для '{hit.SearchTerm}'");
        ...
        private Visio.Shape? DropShapeOnPage(Visio.Page page, string masterName, double xMM, double yMM, int qty, bool isSequential = false)
        {
            ...
            if (mst == null)
            {
                LoggingSystem.Log(LogLevel.WARNING, "Form1", "DropShapeOnPage", $"Мастер '{masterName}' не найден. Открыто документов: {app.Documents.Count}");
                return null;
            }
            ...
            {
                LoggingSystem.Log(LogLevel.ERROR, "Form1", "DropShapeOnPage", $"Ошибка при Drop фигуры '{masterName}': {ex.Message}");
            }
